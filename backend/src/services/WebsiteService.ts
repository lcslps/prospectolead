...
export class WebsiteService {
  ...
  async generateDocument(business: WebsiteDocument['business'], facts: unknown): Promise<WebsiteDocument> {
    ...
    return documentSchema.parse({ name: business.name, business, theme: themeSchema.parse(result.theme), seo: seoSchema.parse(result.seo), sections: result.sections.slice(0, 16).map(s => ({ id: randomUUID(), type: s.type, visible: true, content: contentSchema.parse({ title: s.type === 'header' || s.type === 'footer' ? business.name : s.title, subtitle: s.subtitle, eyebrow: s.eyebrow, text: s.text, items: s.items.filter(item => item.title && [item.title, item.text, item.price].every(value => !value || factStrings.includes(value))), primaryButton: { label: s.primaryLabel, href: business.whatsapp }, secondaryButton: { label: s.secondaryLabel, href: '#contato' } }), settings: settingsSchema.parse(s.type === 'hero' || s.type === 'cta' ? { background: result.theme.primary, color: '#ffffff' } : s.type === 'header' || s.type === 'footer' ? { padding: 24 } : {}) })) });
  }
