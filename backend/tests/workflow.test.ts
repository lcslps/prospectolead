...
test('Prospecção → CRM → Gemini → editor → publicação', async t => {
  ...
  const originalKey = env.GEMINI_API_KEY, originalModel = env.GEMINI_MODEL;
  ...
  try {
    ...
    await t.test('Gemini estruturado cria um único site e move Novo → Site gerado', async () => {
      env.GEMINI_API_KEY = 'test-only-never-transmitted'; env.GEMINI_MODEL = 'test-model';
      globalThis.fetch = async (input, init) => {
        if (String(input).startsWith('https://generativelanguage.googleapis.com/')) {
          ...
          return new Response(JSON.stringify({ candidates: [{ finishReason: 'STOP', content: { parts: [{ text: JSON.stringify({ primary: '#164e63', accent: '#06b6d4', sections: ['header', 'hero', 'menu', 'contact', 'footer'].map(type => ({ type, title: type === 'hero' ? 'Um encontro à mesa' : type, subtitle: '', eyebrow: '', text: '', primaryLabel: '', secondaryLabel: '', items: [{ title: 'Preço inventado', text: '', price: 'R$ 99' }] })) }) }] } }] }), { status: 200 });
        }
        return originalFetch(input, init);
      };
      const result = await request('/websites/generate', { crmLeadId: crmIds[0] }); assert.equal(result.status, 200);
      site = result.body.data; assert.equal(site.generationStatus, 'completed'); assert.equal(site.business.rating, '4.6'); assert.equal(site.business.hours, '');
      ...
      document = { name: site.name, business: site.business, theme: site.theme, sections: site.sections };
    });
