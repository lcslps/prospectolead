...
export function SiteFields({ document: doc, onChange }: { document: SiteDocument; onChange: (doc: SiteDocument) => void }) {
  ...
  const businessLabels: Record<string, string> = { name: 'Nome da empresa', category: 'Categoria', city: 'Cidade / estado', address: 'Endereço', phone: 'Telefone', whatsapp: 'Link do WhatsApp', hours: 'Horários', rating: 'Nota Google', reviewCount: 'Quantidade de avaliações', mapUrl: 'Link do Google Maps' };
  return <>
    ...
    <div className="studio-panel-block"><h3>Ficha do estabelecimento</h3>{Object.entries(businessLabels).map(([key, label]) => <TextField key={key} label={label} value={String(doc.business[key as keyof typeof doc.business] ?? '')} multiline={key === 'hours'} onChange={value => updateBusiness(key, value)} />)}</div>
  </>;
}
export function SectionFields({ section: s, document: doc, onChange }: { section: SiteSection; document: SiteDocument; onChange: (s: SiteSection) => void }) {
  const c = s.content;
  const variants: Partial<Record<SiteSection['type'], string[]>> = { hero: ['background', 'split', 'centered', 'minimal'], services: ['cards3', 'cards2', 'icon-grid', 'list'], about: ['image-left', 'image-right', 'editorial'], gallery: ['grid3', 'grid2', 'mosaic', 'featured'], testimonials: ['cards', 'centered', 'google-rating'], cta: ['accent', 'dark', 'gradient', 'image'], contact: ['split', 'map', 'simple'], team: ['cards3', 'cards4'], prices: ['cards', 'featured'] };
  ...
  return <>
    ...
    <div className="studio-panel-block"><h3>Estilo da seção</h3>{variants[s.type] && <label className="studio-field"><span>Variante</span><Select value={s.variant} onChange={e => onChange({ ...s, variant: e.target.value })}>{variants[s.type]!.map(variant => <option key={variant} value={variant}>{variant}</option>)}</Select></label>}<label className="studio-field"><span>Movimento</span><Select value={s.settings.motion || 'none'} onChange={e => onChange({ ...s, settings: { ...s.settings, motion: e.target.value as 'none' | 'reveal' | 'slide' | 'scale' } })}><option value="none">Nenhum</option><option value="reveal">Revelar</option><option value="slide">Deslizar</option><option value="scale">Escalar</option></Select></label><ColorField label="Fundo" value={s.settings.background} onChange={background => onChange({ ...s, settings: { ...s.settings, background } })} /><ColorField label="Cor do texto" value={s.settings.color} onChange={color => onChange({ ...s, settings: { ...s.settings, color } })} /><label className="studio-field"><span>Alinhamento</span><Select value={s.settings.align} onChange={e => onChange({ ...s, settings: { ...s.settings, align: e.target.value as typeof s.settings.align } })}><option value="left">Esquerda</option><option value="center">Centro</option><option value="right">Direita</option></Select></label>{(['padding', 'radius', 'overlay'] as const).map((key, i) => <label className="studio-field" key={key}><span>{['Espaçamento', 'Bordas das imagens e cards', 'Escurecer imagem de capa'][i]} · {s.settings[key]}</span><Input unstyled type="range" min="0" max={key === 'padding' ? 180 : key === 'radius' ? 80 : 0.9} step={key === 'overlay' ? 0.05 : 1} value={s.settings[key]} onChange={e => onChange({ ...s, settings: { ...s.settings, [key]: Number(e.target.value) } })} /></label>)}</div>
  </>;
}
