...
export function TextField({ label, value, onChange, multiline, field }: { label: string; value: string; onChange: (value: string) => void; multiline?: boolean; field?: string }) {
  return <label className="studio-field"><span>{label}</span>{multiline ? <textarea data-editor-field={field} value={value} onChange={e => onChange(e.target.value)} rows={3} /> : <input data-editor-field={field} value={value} onChange={e => onChange(e.target.value)} />}</label>;
}
function ColorField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) { return <label className="studio-color"><span>{label}</span><input type="color" value={value} onChange={e => onChange(e.target.value)} /><code>{value}</code></label>; }
function ImageField({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  return <div className="studio-image-field">
    ...
    <label className="studio-upload"><Upload size={14} /> Enviar imagem<input type="file" accept="image/png,image/jpeg,image/webp" onChange={e => {
    ...
    {value && <button className="studio-text-button" onClick={() => onChange('')}>Remover imagem</button>}{error && <p role="alert" className="studio-error">{error}</p>}
  </div>;
}
