...
export function LeadWebsite({ crmLeadId, leadId }: { crmLeadId?: string | null; leadId?: string }) {
  ...
  return <section className="panel !p-4"><h3 className="mb-3 flex items-center gap-2 text-sm font-bold text-slate-800 dark:text-white"><Globe size={16} />Site do estabelecimento</h3>
    {!crmId ? <><p className="mb-3 text-xs text-slate-500">Este estabelecimento é um resultado de prospecção. Adicione ao CRM para criar o site.</p><Button variant="unstyled" disabled={busy} className="btn-primary" onClick={() => void add()}>Adicionar ao CRM</Button></> : loading ? <Loader2 className="animate-spin" size={18} /> : site?.generationStatus === 'completed' ? <div className="flex flex-wrap gap-2"><Link className="btn-primary" to={`/studio/${site.id}`}>Editar site</Link><Link className="btn-secondary" to={`/studio/${site.id}?preview=1`}>Visualizar</Link><Link className="btn-secondary" to={`/studio/${site.id}`}>Publicar</Link>{site.status === 'PUBLISHED' && <a className="btn-secondary" href={`/s/${site.id}`} target="_blank" rel="noreferrer">Site publicado ↗</a>}</div> : <>
      ...
      {working && <div className="mt-4 space-y-2 text-xs text-slate-500" role="status">{waitingRetry ? <><p className="flex gap-2"><Loader2 size={14} className="animate-spin" />Gemini indisponível no momento. A geração será retomada automaticamente.</p><p>Você pode deixar esta tela aberta; o status será atualizado.</p></> : <><p className="flex gap-2"><Check size={14} />Dados do estabelecimento preparados</p><p className="flex gap-2"><Loader2 size={14} className="animate-spin" />IA analisando o negócio, definindo direção de arte e design system...</p><p className="flex gap-2"><Loader2 size={14} className="animate-spin" />Escolhendo componentes, gerando e validando o site...</p><p>O site será salvo e aberto no editor quando estiver pronto.</p></>}</div>}
