...
export function WebsiteRenderer({ document: doc, selectedId, onSelect, interactive = true }: { document: SiteDocument; selectedId?: string | null; onSelect?: (id: string, field?: string) => void; interactive?: boolean }) {
  ...
  return <main className="website-renderer" style={{ '--ws-primary': doc.theme.primary, '--ws-accent': doc.theme.accent, '--ws-radius': `${doc.theme.radius}px`, background: doc.theme.background, color: doc.theme.text, fontFamily: doc.theme.font === 'serif' ? 'Georgia, serif' : 'Arial, sans-serif' } as CSSProperties}>
    {doc.sections.filter(s => s.visible).map(s => {
      ...
      return <section key={s.id} id={s.id} data-section-id={s.id} className={`ws-section ws-${s.type} ws-variant-${s.variant} ws-motion-${s.settings.motion || 'none'} ${selectedId === s.id && !interactive ? 'ws-selected' : ''} ${!interactive ? 'ws-editable' : ''}`} onClick={e => handleClick(e, s)} style={{ backgroundColor: s.settings.background, color: s.settings.color, textAlign: s.settings.align, padding: `${s.settings.padding}px clamp(20px, 5vw, 72px)`, ...(hero && bg && s.variant !== 'minimal' ? { backgroundImage: `linear-gradient(rgba(0,0,0,${s.settings.overlay}),rgba(0,0,0,${s.settings.overlay})), url("${bg.replaceAll('"', '%22')}")` } : {}) }}>
        {s.type === 'contact' && doc.sections.find(x => x.type === 'contact' && x.visible)?.id === s.id && <span id="contato" />}
        <div className="ws-inner">
          {s.type === 'header' ? <div className="ws-navigation"><strong data-field="title">{c.title || b.name}</strong><details className="ws-menu"><summary aria-label="Abrir menu">☰</summary><nav>{doc.sections.filter(x => x.visible && !['header', 'hero', 'footer'].includes(x.type)).slice(0, 5).map(x => <a key={x.id} href={`#${x.id}`}>{x.content.title}</a>)}</nav></details><CTA button={c.primaryButton} /></div> : <>
