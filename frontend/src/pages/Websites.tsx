...
export function WebsitesPage() {
  ...
  return <div>
    <div className="workspace-heading">
      ...
      <button className="btn-primary" onClick={() => void openCreate()}><Plus size={16} />Novo projeto</button>
    </div>
    {error && <div role="alert" className="mb-5 flex items-center justify-between gap-4 rounded-lg bg-red-50 p-4 text-sm text-red-700">{error}<button onClick={() => void load()} className="underline">Tentar novamente</button></div>}
    <div className="project-tabs" role="tablist" aria-label="Filtrar projetos">{(Object.entries(filterLabels) as [ProjectFilter, string][]).map(([key, label]) => <button key={key} role="tab" aria-selected={filter === key} onClick={() => setFilter(key)}>{label}<span>{projects.filter(p => matchesFilter(p, key)).length}</span></button>)}</div>
    <div className="projects-toolbar"><label className="projects-search"><Search size={15} /><input className="input" aria-label="Buscar projetos" placeholder="Buscar por nome ou empresa" value={search} onChange={e => setSearch(e.target.value)} /></label><span>{shown.length} {shown.length === 1 ? 'projeto' : 'projetos'} · mais recentes primeiro</span><button className="btn-secondary !px-3" aria-label="Atualizar projetos" onClick={() => void load()}><RefreshCw size={14} /></button></div>
    {loading ? <PageLoader /> : shown.length === 0 ? projects.length > 0 ? <div className="card"><EmptyState title="Nenhum projeto por aqui" description="Tente outro nome ou escolha uma categoria diferente." action={<button className="btn-secondary" onClick={() => { setSearch(''); setFilter('all'); }}>Limpar filtros</button>} /></div> : <div className="projects-empty">
      <div className="projects-empty-copy"><span aria-hidden="true">Seu próximo projeto.</span><h3>Todo negócio merece<br />um bom lugar na internet.</h3><p>Escolha uma empresa do seu CRM e comece a criar. Seus rascunhos e sites publicados ficam organizados aqui.</p><button className="btn-primary" onClick={() => void openCreate()}>Criar meu primeiro projeto<ArrowUpRight size={15} /></button></div>
