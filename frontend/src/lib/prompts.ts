...
11. LOCALIZAÇÃO COM MAPA VISÍVEL NA PÁGINA: havendo endereço, cidade ou link real do Google Maps, a seção de localização mostra o mapa INCORPORADO (não só um botão/link): um <iframe> do Google Maps sem API key + endereço legível + botão "Ver localização" abrindo o mapa real em nova aba. Monte o iframe assim: <iframe src="https://maps.google.com/maps?q=<ENDEREÇO_OU_CIDADE_CODIFICADO_PARA_URL>&t=&z=15&ie=UTF8&iwloc=&output=embed" loading="lazy" title="Mapa"></iframe> — troque o q pelos dados reais do briefing com codificação de URL (espaço vira +, vírgula vira %2C; ex: Av.+Brasil%2C+100+-+Sorriso%2C+MT). Estilize com CSS (largura total, altura ~380–450px, borda sutil ou filtro em harmonia com a paleta, sem quebrar o layout no mobile). Nunca invente endereço: sem dado real de localização, não mostre mapa nem botão.
...
- LARGURA TOTAL OBRIGATÓRIA: o documento deve ocupar 100% da largura da viewport em desktop e mobile. Defina html e body com width: 100%, min-width: 0 e margin: 0; não aplique max-width, width fixa, margem horizontal automática ou padding externo ao body, main, ao hero ou ao wrapper raiz da página. O hero, fundos de seção e imagens de faixa devem ir de uma borda à outra da viewport (width: 100% ou 100vw). Somente blocos internos de leitura, como classes container/content, podem ter max-width e margin auto. Antes de responder, confirme que não haverá faixas vazias nas laterais em telas largas.
 - CHECKLIST OBRIGATÓRIO ANTES DE RESPONDER: entregue o HTML inteiro e fechado; mantenha o header e o hero como contrato fixo e construa as demais seções que a história desta empresa pede. Contato/conversão: exiba telefone/WhatsApp e cidade somente quando existirem (tel:+55... e https://wa.me/55... com mensagem pré-preenchida natural, só com contato real); sem contato, CTA de âncora para formulário local honesto com feedback local. Avaliações: nota real sem inflar, nunca depoimento inventado. Localização: mapa Google INCORPORADO na página (iframe com output=embed e endereço real) + endereço legível + "Ver localização"; sem dado real, sem mapa. Confira ainda: parece site de designer ou template de IA? hero impressiona em 3 segundos? proposta e ação claras em 5 segundos? identidade própria do segmento (não repetir fórmulas)? títulos serifados + corpo sans? paleta do segmento em variáveis? imagens coerentes com o negócio? CTAs com verbo específico? sem emoji-ícone, área vazia, placeholder ou dado inventado? contraste corpo ≥4.5:1, ::selection e focus-visible na paleta? Se falhar, refaça.
...
Link do Google Maps: ${lead.googleMapsUri || '(não informado)'}
${lead.address || lead.city || lead.googleMapsUri ? `Mapa incorporado OBRIGATÓRIO: inclua <iframe src="https://maps.google.com/maps?q=${encodeURIComponent([lead.address, lead.city, lead.state].filter(Boolean).join(', '))}&t=&z=15&ie=UTF8&iwloc=&output=embed" loading="lazy"></iframe> (endereço real acima, já codificado).` : 'Sem dados de localização — NÃO mostre mapa nem botão de localização.'}
`
    : '';
...
export function validateGeneratedSite(site: ParsedSite): SiteValidation {
  ...
  if (!/<iframe\b[^>]*\bsrc=["'][^"']*maps\.google\.com\/maps[^"']*output=embed[^"']*["']/i.test(html)) {
    warnings.push('mapa do Google não incorporado (iframe com output=embed ausente)');
  }
